...
-- Safe to run multiple times; uses IF NOT EXISTS for tables and extensions.

-- Extensions
create extension if not exists "pgcrypto";

-- Core agents/tenants table (alias view provided below)
...
-- Backward-compatible view for existing code paths
...
create index if not exists idx_phone_mappings_agent on public.phone_mappings(agent_id);
...
create index if not exists idx_leads_agent_phone on public.leads(agent_id, phone_number);
...
create index if not exists idx_interactions_agent on public.interactions(agent_id);
...
create index if not exists idx_discord_logs_agent on public.discord_logs(agent_id);
...
create policy if not exists agents_update_self on public.agents
  for update using (auth.uid() = id or auth.uid() = user_id)
  with check (auth.uid() = id or auth.uid() = user_id);

-- Phone mappings tied to owning agent
create policy if not exists phone_mappings_select_owner on public.phone_mappings
  for select using (auth.uid() = agent_id);
create policy if not exists phone_mappings_ins_upd_owner on public.phone_mappings
  for insert with check (auth.uid() = agent_id);
create policy if not exists phone_mappings_update_owner on public.phone_mappings
  for update using (auth.uid() = agent_id) with check (auth.uid() = agent_id);

-- Leads scoped to agent
create policy if not exists leads_select_owner on public.leads
  for select using (auth.uid() = agent_id);
create policy if not exists leads_ins_owner on public.leads
  for insert with check (auth.uid() = agent_id);
create policy if not exists leads_update_owner on public.leads
  for update using (auth.uid() = agent_id) with check (auth.uid() = agent_id);

-- Properties scoped to agent
create policy if not exists properties_select_owner on public.properties
  for select using (auth.uid() = agent_id);
create policy if not exists properties_ins_owner on public.properties
  for insert with check (auth.uid() = agent_id);
create policy if not exists properties_update_owner on public.properties
  for update using (auth.uid() = agent_id) with check (auth.uid() = agent_id);

-- Interactions scoped to agent
create policy if not exists interactions_select_owner on public.interactions
  for select using (auth.uid() = agent_id);
create policy if not exists interactions_ins_owner on public.interactions
  for insert with check (auth.uid() = agent_id);
create policy if not exists interactions_update_owner on public.interactions
  for update using (auth.uid() = agent_id) with check (auth.uid() = agent_id);

-- Discord logs scoped to agent
create policy if not exists discord_logs_select_owner on public.discord_logs
  for select using (auth.uid() = agent_id);
create policy if not exists discord_logs_ins_owner on public.discord_logs
  for insert with check (auth.uid() = agent_id);
create policy if not exists discord_logs_update_owner on public.discord_logs
  for update using (auth.uid() = agent_id) with check (auth.uid() = agent_id);
